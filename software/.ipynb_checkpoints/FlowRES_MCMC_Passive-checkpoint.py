...
def Chain_Initialiser_Passive(Net, tran_coeff, buffer_size=10000):
    Noise = np.random.normal(0, tran_coeff, size=(buffer_size, Net.max_len, 2))
    Starts = np.tile(Net.start[:2], (buffer_size,1,1))
...
class FlowRes_MCMC():
    ...
    def Explore(self, iterations, epochs=1, return_chains=False, return_hists=False, hist_bins=None, save_to_file=False, save_net=False, file_loc=None, save_each=1):
        ...
        x_pos = self.initial_chains
        if return_chains:
            chains = np.zeros((iterations+1, self.num_chains, self.Net.max_len, 2))
            chains[0] = x_pos
        if return_hists:
        ...
        for itr in range(iterations):
            ...
            if save_to_file:
                if ((itr+1) % save_each == 0) or ((itr+1) == iterations): # Save regularly so that even if training is interrupted, we have results stored
                    ...
                    if return_hists:
                        np.save(file_loc + '_hists', all_hists[:itr+2].astype('float32'))
                    # Times and Metrics# 
